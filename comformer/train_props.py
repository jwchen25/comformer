...
For custom dataset training, use train_custom_icomformer() from comformer.custom_train module.
...
def train_prop_model(
    prop="",
    dataset="custom",  # Only "custom" is supported now
    ...
    Use train_custom_icomformer() from comformer.custom_train module instead.

    Example:
        from comformer.custom_train import train_custom_icomformer
        results = train_custom_icomformer(strucs=strucs, labels=labels)
    """
    raise DeprecationWarning(
        "train_prop_model() is deprecated. "
        "Use train_custom_icomformer() from comformer.custom_train module for custom datasets. "
        "Predefined datasets (dft_3d, megnet, mpf) are no longer supported."
    )
